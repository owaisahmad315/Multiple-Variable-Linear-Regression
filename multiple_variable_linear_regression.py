...
"""
Compute Gradient with Multiple Variables An implementation for calculating the equations (6) and (7) is below. There are many ways to implement this. In this version, there is an

outer loop over all m examples. ∂𝐽(𝐰,𝑏)∂𝑏 for the example can be computed directly and accumulated in a second loop over all n features: ∂𝐽(𝐰,𝑏)∂𝑤𝑗 is computed for each 𝑤𝑗 .
